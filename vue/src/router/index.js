...
const routes = [
  {
    ...
    children: [
      ...
      { path: '/return', name: 'Return', meta: { name: '退换货管理' }, component: () => import('@/views/manager/Return.vue') },
    ]
  },
  {
    path: '/front',
...
router.beforeEach((to, from, next) => {
  ...
  } else if (to.path === '/home' && user.role === 'USER') {
    next('/front/home');
  } else if (to.path === '/front/home' && user.role !== 'USER') {
    next('/home');
  } else {
    next();
  }
